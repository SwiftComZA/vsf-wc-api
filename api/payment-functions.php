...
// *************************************************************
//  This prepares the cart to create an order, checking if it's valid etc
// *************************************************************
function vsf_create_order($request)
{
  // Check if cart is empty
  $cart = WC()->cart;
  if ($cart->is_empty()) {
      return new WP_Error('bad_cart', 'You cannot check out an empty cart.', array('status' => 400));
  }
  // Check cart item validity
  $result = $cart->check_cart_item_validity();
  if (is_wp_error($result)) {
      return new WP_Error('bad_cart_items', $result, array('status' => 400));
  }
  // Check cart item stock levels
  $result = $cart->check_cart_item_stock();
  if (is_wp_error($result)) {
      return new WP_Error('bad_cart_stock', $result, array('status' => 400));
  }
  ...
  if ($cart_total != $request_total) {
      return new WP_Error('bad_cart_meta', 'Cart totals do not match', array('status' => 400));
  }

  // Convert cart to actual order and return order data
  return vsf_convert_cart_to_order($request);
}

// *************************************************************
//  This converts the cart to an order
// *************************************************************
function vsf_convert_cart_to_order($request)
{
...
// *************************************************************
//  Setup payment on an order
// *************************************************************
function vsf_init_order_payment($order, $request)
{
  $user_id = get_current_user_id();
  ...
  switch ($request['paymentMethod']) {
    case 'bacs':
        ...
        if (empty($gateways['bacs'])) {
            return new WP_Error('bad_payment_gateway', 'The bacs payment gateway is not enabled.', array('status' => 500));
        }
        break;
  }
  ...
  // Save to database
  $order->save();
  if (is_wp_error($response_data) && isset($response_data->get_error_data()['notification']) && !isset($response_data->get_error_data()['not_critical'])) {
  ...
  if (is_array($response_data)) {
      $response_data['order'] = vsf_build_order_object($order);
...
// *************************************************************
//  This builds the order object so it's consistent
// *************************************************************
function vsf_build_order_object($order)
{
    $gateways = WC()->payment_gateways->get_available_payment_gateways();
